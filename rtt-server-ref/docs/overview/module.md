...
This function is called when the client has connected to the server,
and provides the game setup information: scenario, options, and any
static data that will remain constant for the entire session but
changes from game to game (for example a randomized map).
...
### Static View
	...
	exports.static_view = function (game) {
		// return any static information the client needs for this game
	}
