...
app.post("/account/password/reset", function (req, res) {
	...
	var ip = req.headers["x-real-ip"] || req.ip || req.connection.remoteAddress || "0.0.0.0"
...
const SQL_INSERT_CONTACT_NOTE = SQL("insert into contacts (me,you,relation,note) values (?,?,?,?)")
...
function check_join_game_limit(user, n) {
	if (user.waiting_games >= LIMIT_WAITING_GAMES)
		return "There are games waiting for you to move!"
	...
	var queued = TM_COUNT_QUEUE_GAMES.get(user.user_id) / 2
	if (active + queued + n > Math.max(LIMIT_ACTIVE_GAMES_MIN, Math.min(LIMIT_ACTIVE_GAMES_MAX, finished)))
...
function check_join_seed_limit(user, seed) {
	let limit = check_join_game_limit(user, seed.round_count / 2)
	if (limit)
		return limit
...
const TM_SEED_LIST_ALL = SQL(`
	...
	where is_open or q1 > 0 or q2 > 0 or q3 > 0
	order by seed_name
`)
...
app.get("/tm/list", must_be_logged_in, function (req, res) {
